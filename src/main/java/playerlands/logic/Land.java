...
import playerlands.api.PlayerlandsAPI;
...
public class Land {
	MinecraftServer server = Playerlands.instance.server;
	Fantasy fantasy = Playerlands.instance.fantasy;
	...
	public PlayerPosition spawnPos = Playerlands.config.defaultSpawnPos;
	public PlayerPosition visitsPos = Playerlands.config.defaultVisitsPos;
	...
	public List<ServerPlayerEntity> getPlayers() {
		...
		return server.getPlayerManager().getPlayerList().stream().filter(player -> {
			var land = PlayerlandsAPI.getLand(player);
	...
	public RuntimeWorldHandle getHandle() {
		...
		RuntimeWorldHandle handle = this.fantasy.getOrOpenPersistentWorld(PlayerlandsMod.id(this.owner.uuid.toString()), this.landConfig);
		...
		if(!isMember(visitor)) {
			this.getOwner().ifPresent(owner -> {
				if(!visitor.getUuid().equals(owner.getUuid())) {
					owner.sendMessage(PlayerlandsTexts.prefixed("message.playerlands.land_visit.visit", map -> map.put("%visitor%", visitor.getName().getString())));
				}
			});
		}

		PlayerlandsAPI.ON_LAND_VISIT.invoker().invoke(visitor, world, this);
	...
	public void onFirstLoad(PlayerEntity player) {
		...
		PlayerlandsAPI.ON_LAND_FIRST_LOAD.invoker().invoke(player, world, this);
	}
}
